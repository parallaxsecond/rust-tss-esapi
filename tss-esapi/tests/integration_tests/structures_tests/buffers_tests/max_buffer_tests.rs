...
#[test]
fn test_max_sized_max_buffer_conversions() {
    let expected_buffer = [0xffu8; MaxBuffer::MAX_SIZE];
    let native = MaxBuffer::try_from(expected_buffer.as_slice().to_vec())
        .expect("It should be possible to convert an array of MAX size into a MaxBuffer object.");
    let tss = TPM2B_MAX_BUFFER::from(native);
    assert_eq!(MaxBuffer::MAX_SIZE, tss.size as usize);
