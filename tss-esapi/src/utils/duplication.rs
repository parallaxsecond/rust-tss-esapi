    ...
    PSymAlg: BlockCipherEncrypt + KeyInit,
{
        ...
        NameAlg: Digest,
    {
        ...
        let mut enc_sensitive: Zeroizing<Box<[u8]>> = Zeroizing::new({
            let mut out = Vec::with_capacity(inner_integrity.len() + Sensitive::BUFFER_SIZE);
            out.extend_from_slice(&inner_integrity);
            out.append(&mut sensitive_priv.marshall()?);
            out.to_vec().into()
        });
        let iv = {
...
fn outer_wrapper<NpNameAlg, NpSymAlg>(
    seed: &Key<NpSymAlg>,
    ...
    <<<NpNameAlg as EagerHash>::Core as OutputSizeUser>::OutputSize as Mul<U8>>::Output: Unsigned,
{
    ...
    let sym_key = kdfa::<NpNameAlg, kdf::Storage, NpSymAlg>(seed, sensitive_name.value(), &[])?;
    ...
    let hmac_key = kdfa::<NpNameAlg, kdf::Integrity, NpSymAlg>(seed, &[], &[])?;
    ...
    let mut out = Vec::with_capacity(
        ( u16::BITS / 8) as usize // TODO: why do I need the size again?!

        + OutputSize::<Hmac<NpNameAlg>>::USIZE * 2 + enc_sensitive.len(),
    );
    out.extend_from_slice(outer_hmac.as_bytes());
    out.extend_from_slice(enc_sensitive);

    Private::from_bytes(&out)
}
    ...
    R: CryptoRng + ?Sized,
{
        ...
        <NpSymAlg::KeySize as Mul<U8>>::Output: Unsigned,
    {
        ...
        let (seed, seed_ciphertext) = encapsulate::<R, NpSymAlg>(rng, new_parent)?;

        let object_name = sensitive_kp.0.name()?;
        ...
        let InnerWrapper {
            ...
            SymmetricDefinitionObject::Null => {
                ...
                let enc_sensitive = Zeroizing::new(sensitive_kp.1.marshall()?.into());
                Ok(InnerWrapper {
                    sym_key: None,
                    enc_sensitive,
                })
        ...
        macro_rules! match_inner {
            ($hash: ty) => {
                outer_wrapper::<$hash, NpSymAlg>(&seed, &mut enc_sensitive, object_name)
            };
        }

        let payload = super::match_name_hashing_algorithm!(new_parent, match_inner)?;
