...
use crate::{Error, Result, constants::return_code::TpmFormatOneError};
...
impl std::fmt::Display for TpmFormatOneResponseCode {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self.error_number {
            ...
            TpmFormatOneError::Binding => write!(
                f,
                "Public and sensitive portions of an object are not cryptographically bound ({}).",
                self.argument_number()
            ),
            TpmFormatOneError::Curve => {
                write!(f, "Curve not supported ({}).", self.argument_number())
            }
            TpmFormatOneError::EccPoint => write!(
                f,
