...
pub mod target {
    ...
    pub fn ensure_supported() {
        ...
        match (target.architecture, target.operating_system) {
            ...
            (arch, os) => {
                panic!(
                    "Compilation target (architecture, OS) tuple ({}, {}) is not part \
                    of the supported tuples. Please compile with the \"generate-bindings\" \
                    feature or add support for your platform.",
                    arch, os
                );
            }
        }
    ...
    impl Installation {
        ...
        fn ensure_valid_installation_path(env_var: String) -> (PathBuf, String) {
            ...
            if !install_path.join("include").is_dir() {
                panic!(
                    "`{}` specifies a path `{}`, that does not contain an `include` directory",
                    PATH_ENV_VAR_NAME,
                    install_path.to_string_lossy()
                );
            }
            if !install_path.join("lib").is_dir() {
                panic!(
                    "`{}` specifies a path `{}`, that does not contain an `lib` directory",
                    PATH_ENV_VAR_NAME,
                    install_path.to_string_lossy()
                );
            }
            ...
            if !min_version_req.matches(&version) {
                panic!(
                    "Minimum supported version is {}, found installed version {}",
                    MINIMUM_VERSION, version_str
                );
            }
            println!(
